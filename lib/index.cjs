...
const index_js = require('@waxio/waxjs/dist/index.js');

class UALWaxError extends universalAuthenticatorLibrary.UALError {
  constructor(message, type, cause) {
    super(message, type, cause, "Wax");
  }
}

class WaxUser extends universalAuthenticatorLibrary.User {
  ...
  async signTransaction(transaction, options) {
    try {
      ...
      throw new UALWaxError(e.message ? e.message : "Unable to sign transaction", universalAuthenticatorLibrary.UALErrorType.Signing, e);
    }
  }
  async signArbitrary() {
    throw new UALWaxError("WAX Cloud Wallet does not currently support signArbitrary", universalAuthenticatorLibrary.UALErrorType.Unsupported, null);
  }
  async verifyKeyOwnership() {
    throw new UALWaxError("WAX Cloud Wallet does not currently support verifyKeyOwnership", universalAuthenticatorLibrary.UALErrorType.Unsupported, null);
  }
...
class Wax extends universalAuthenticatorLibrary.Authenticator {
  ...
  async login() {
    ...
    if (this.chains.length > 1) {
      throw new UALWaxError("WAX Could Wallet only supports one WAX chain", universalAuthenticatorLibrary.UALErrorType.Unsupported, null);
    }
    if (!this.wax) {
      throw new UALWaxError("WAX Cloud Wallet not initialized yet", universalAuthenticatorLibrary.UALErrorType.Initialization, null);
    }
    try {
      if (!this.session) {
      ...
      throw new UALWaxError(e.message ? e.message : "Could not login to the WAX Cloud Wallet", universalAuthenticatorLibrary.UALErrorType.Login, e);
    }
  }
  async logout() {
    this.initWaxJS();
